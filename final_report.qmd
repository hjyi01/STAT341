...
Measures: The interest rate that commercial banks lend money to one another overnight at

Original Unit of measure: percent

Standardized: yes

**Brazil Recession Indicator**

Measures: Whether or not Brazil is in a recession

Original units of measure: 0 - no recession and 1 - recession

Standardized: no

**US Job Postings**

Measures: A seasonally adjusted number of new US job postings on Indeed

Original unit of measure: Indexed at February, 1 2020 = 100
...
$$ \text{Reals USD Exchange} \sim \text{Gamma distribution}(\mu_{[i]}, \sigma) $$

$$ \mu_{[i]} = \beta_0+\beta_1*\text{FedFundsRate}+\beta_{\text{RecessionIndicator[i]}}+\beta_3*\text{JobPostings} $$
...
prior_pred_dist <- tibble(
  ...
  rowwise()  |>
  mutate(
    mu = list(exp(b0 + b1 * exchange$FedFundsRate_scaled + b2 * exchange$RecessionIndicator + b3 * exchange$JobPostings_scaled))) |>
  unnest(cols = mu) |> 
  ungroup() |>
  mutate(
    alpha = mu^2 / sigma^2,
  ...
  rowwise() |>
  mutate(
    y_sim = rgamma(1, shape = alpha, rate = lambda)
  ) |>
  ungroup()
```

```{r}
gf_dens(~ y_sim, data = prior_pred_dist, color = "darkblue") |>
  gf_labs(
...
The prior predictive distribution is centered at zero, which suggests our priors are placing more weight on the lower tail of our distribution (meaning it is expecting an exchange rate close to zero). Alternatively, one or more of our priors could have been better-defined. We have learned that choosing GLM priors is difficult.
