...
exchange <- exchange |>   
  ...
  mutate(FedFundsRate_scaled = as.numeric(scale(FedFundsRate)),          
         ...
         Exchange_scaled = as.numeric(scale(Reals_USD_Exchange)),
         RecessionIndicator = if_else(RecessionIndicator == "0", "1", "2"))
```

```{r}
exchange <- exchange |>
  select(FedFundsRate_scaled, JobPostings_scaled, Exchange_scaled, RecessionIndicator)
```
...
gf_dens(~ y_sim, data = prior_pred_dists) %>%
  gf_labs(
...
```{r}
exchange_fitted_model <- stan(file = 'final-project.stan',
                       data = exchange,
                       chains = 4,
                       iter = 2000,
                       warmup = floor(iter/2),
